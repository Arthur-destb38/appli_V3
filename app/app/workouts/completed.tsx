...
export default function CompletedWorkoutsScreen() {
  ...
  // Filtrer les séances complétées
  const completedWorkouts = useMemo(() => {
    return workouts.filter((item) => item.workout.status === 'completed');
  }, [workouts]);
      ...
      >
        <WorkoutCard
          ...
          onPress={() => {
            ...
            router.push(`/history/${item.workout.id}` as never);
          }}
        />
      </Animated.View>
    );
    ...
    return (
      <View style={[styles.container, { backgroundColor: theme.colors.background, paddingTop: insets.top }]}>
        <View style={styles.header}>
          ...
          <Text style={[styles.title, { color: theme.colors.textPrimary }]}>Mes séances passées</Text>
          <View style={{ width: 24 }} />
        </View>
          ...
          <Animated.View style={{ opacity: fadeAnim }}>
            <AppCard>
              <View style={styles.emptyState}>
                <Ionicons name="checkmark-circle-outline" size={72} color={theme.colors.textSecondary} />
                <Text style={[styles.emptyTitle, { color: theme.colors.textPrimary }]}>
                  Aucune séance terminée
                </Text>
                <Text style={[styles.emptySubtitle, { color: theme.colors.textSecondary }]}>
                  Termine une séance pour voir ton historique ici
                </Text>
              </View>
            </AppCard>
          </Animated.View>
